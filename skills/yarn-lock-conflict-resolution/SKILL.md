...
description: Use this skill when resolving merge/rebase conflicts in yarn.lock. It standardizes taking yarn.lock from the integration target branch, then running yarn install, and reusing the previous successful resolution as the base for repeated conflict rounds.
---
...
- During rebase, always take `yarn.lock` from the target branch (the branch you
  rebase onto).
- During merge, always take `yarn.lock` from the current target branch (`HEAD`).
- After taking that baseline, run `yarn install` to reconcile lock metadata with
  current manifests.
...
- For repeated conflict rounds, use previous successful `yarn.lock` resolution
  as the new base.
- Exception for intentional dependency updates (`yarn up`, `yarn add`,
  `yarn remove`, etc.):
  after conflict resolution, rerun the original dependency command and create a
  fresh `chore: Updated yarn.lock` commit.
...
```
7. If current rebased change intentionally updated dependencies, replay that
intent:
```bash
# example
yarn up <packages>
