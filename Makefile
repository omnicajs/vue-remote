...
.PHONY: compose-check
compose-check: ## Checks Docker Compose files
	$(TARGET_HEADER)
	node scripts/compose-check.mjs compose.yml
	$(COMPOSE) --profile '*' config >/dev/null

.PHONY: node_modules
node_modules: compose-check package.json yarn.lock ## Installs dependencies
	$(TARGET_HEADER)
	@$(COMPOSE) run --rm node yarn install --silent
	@touch node_modules || true

.PHONY: build
build: compose-check ## Builds the package
	$(TARGET_HEADER)
	$(YARN) build

.PHONY: release
release: compose-check ## Bumps version and creates tag
	$(TARGET_HEADER)
ifdef as
	$(YARN) release:$(as)
else
...
.PHONY: tests
tests: compose-check ## Runs autotests
	$(TARGET_HEADER)
ifdef cli
...
tests-coverage: compose-check ## Runs unit/integration tests with coverage
	$(TARGET_HEADER)
ifdef cli
...
.PHONY: tests-e2e
tests-e2e: compose-check ## Runs new browser e2e tests (vitest + playwright provider)
	$(TARGET_HEADER)
ifdef cli
...
tests-e2e-coverage: compose-check ## Runs browser e2e tests with coverage
	$(TARGET_HEADER)
ifdef cli
...
tests-all-coverage: compose-check ## Runs full merged coverage (unit/integration + browser e2e)
	$(TARGET_HEADER)
ifdef cli
...
.PHONY: tsc
tsc: compose-check ## Runs all TypeScript checks
	$(TARGET_HEADER)
	$(YARN) typecheck
